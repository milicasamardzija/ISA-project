...
export default ({
    ...
    methods: {
        ...
        async sendMails(content) {
            this.counter = 0;
            this.content = content;
            const headers = {
                Authorization: "Bearer " + localStorage.getItem("token"),
            };
            axios.post("http://localhost:8081/api/complaint/sendMails/"+this.emailWhoSent+"/"+this.emailWhoReceive + "/"+ this.content,{headers})
            .then (response => { 
