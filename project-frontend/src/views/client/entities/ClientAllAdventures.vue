...
<div v-if="this.role === 'ROLE_CLIENT'">
  ...
  <div class="filter" style="margin-left:20px;margin-top: 20px">
    <div class="dropdown">
        ...
        aria-haspopup="true" aria-expanded="false" v-if="adventures.length != 0">
          ...
          <div class="col-with-picture">
            <div v-if="adventure.images.length != 0">
              ...
              style="margin-top: 1rem; max-height: 300px !important; max-width: 320px !important"
               @click="goToAdventure(adventure.nameOfAdventure),getSelected(adventure)"
            />
            </div>
          </div>
          <div class="col-info">
...
export default {
  ...
  methods: {
    ...
    async filterData(param){
     ...
     async showAdventure(a){
        this.$router.push({ name: 'MyService', params: { id: a.id}})
    },
     goToCottages(){
      //this.closeModal();
      this.r = false;
    ...
    getNumberPeople(){
       const headers = {
        ...
        axios.get(process.env.VUE_APP_BACKEND_URL+"/api/adventures/getMaxPeople/" + this.selectedEntity.id ,{headers})
        .then (response => { 
          console.log(response.data);
    ...
    var dt = this.format_date(this.date)
     if (this.date == "" || this.time == "" || this.number == "") {
      ...
      axios.post(process.env.VUE_APP_BACKEND_URL+"/api/adventures/reservationSearch" ,{
          date: this.date,
          time: this.time,
          city: this.city,
